        ...
        static void Postfix()
        {
            ...
            try
            {
                ...
                if (player != null)
                {
                    ...
                    Vector3i posI = World.worldToBlockPos(player.position);
                    ...
                    // The player free-falls from spawn to bedrock, so the client
                    // sees no collision under it. Dump the client's own column
                    // around the player: if these are air while the server says
                    // dirt/stone, our NetPackageChunk content is the defect.
                    Vector3i b = posI;
                    string col = "";
                    ...
                    // Screenshots show the loading artwork while the ID checks
                    // above all report closed, so enumerate what is actually
                    // open instead of guessing IDs. frameCount tells apart "no
                    // new frames rendered" from "loading screen still up".
                    // openWindows lists only "toolTip", which is not what an
                    // in-game client looks like, yet the screen shows loading
                    // artwork. Ask every registered window whether it is
