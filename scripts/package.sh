#!/usr/bin/env bash
# Build the mod and package dist/7dtd-connect into a distributable zip.
#
# The zip contains the 7dtd-connect/ mod folder at its top level, so
# unzipping it inside <game>/Mods installs the mod (Mods/7dtd-connect/).
#
...
OUT="$ROOT/dist/7dtd-connect-$VERSION.zip"
STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT
cp -a "$ROOT/dist/7dtd-connect" "$STAGE/"
( cd "$STAGE" && zip -qr "$OUT" 7dtd-connect )
echo "Packaged -> $OUT"
